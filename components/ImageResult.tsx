...
export const ImageResult: React.FC<ImageResultProps> = ({ imageUrl, isLoading, language, error, onEdit }) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="relative w-full h-96 min-h-[260px] rounded-2xl flex flex-col items-center justify-center transition-all duration-300 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden">
          ...
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900">
            <div className="flex flex-col items-center justify-center gap-[12px]">
                <div className="relative">
                  ...
                  <div className="w-[44px] h-[44px] border-[3px] border-slate-200 dark:border-slate-800 border-t-violet-500 rounded-full animate-spin"></div>
                  {/* Glow Effect */}
                  <div className="absolute inset-0 bg-violet-500/20 blur-xl rounded-full"></div>
                </div>
                <p className="text-slate-600 dark:text-slate-300 text-center max-w-xs font-medium text-base opacity-85 shadow-black drop-shadow-md">
                  {t.loadingDesc}
                </p>
            </div>
          </div>
        ...
        {/* Error Layer */}
        {!isLoading && error && (
          <div className="absolute inset-0 z-0 flex flex-col items-center justify-center p-8 bg-red-50 dark:bg-red-950/10">
            <div className="w-16 h-16 bg-red-100 dark:bg-red-900/20 rounded-full flex items-center justify-center mb-4">
                <AlertCircle className="w-8 h-8 text-red-500" />
            </div>
            <h3 className="text-lg font-medium text-red-600 dark:text-red-200 mb-2">Generation Failed</h3>
            <p className="text-red-500 dark:text-red-400 text-center max-w-xs text-sm">
              {error}
            </p>
          </div>
        )}
        ...
        {!isLoading && !error && !imageUrl && (
          <div className="absolute inset-0 z-0 flex flex-col items-center justify-center p-8 bg-slate-50/50 dark:bg-slate-900/50 border-dashed border-slate-200 dark:border-slate-800">
            ...
            <h3 className="text-xl font-medium text-slate-700 dark:text-slate-300 mb-2">{t.readyToCreate}</h3>
